    ...
    with pdfplumber.open(pdf_path) as pdf:
        for i, page in enumerate(pdf.pages):
            ...
            for j, img in enumerate(images):
                img_path = os.path.join(output_folder + "/", f"{i}{j}.png")
...
def extract_data_from_pdf(pdf_path):
    ...
    img.anchor = "A2"  # Set the anchor point to cells A1 to F1
    img.width = 200
    img.height = 200

    new_worksheet.add_image(img)

    # Populate other columns
    """ for ref, des, color, qty in zip(
        supplier_reference, supplier_designation, colours, qty_available
    ):
        ws.append(
            [
                "",
...
def extract_and_display():
    ...
    messagebox.showinfo(
        "Info", f"Extraction complete. Output file saved as:\n{output_path}"
    )
...
extract_button = tk.Button(
    root, text="Extract and Display Data", command=extract_and_display
)
